...
DROP FUNCTION IF EXISTS `isismigration`.`HTML_UnEncode`$$
CREATE DEFINER=`root`@`localhost` FUNCTION `HTML_UnEncode`(x TEXT) RETURNS TEXT CHARSET latin1
BEGIN
...
IF INSTR( x , '&quest;' )
THEN SET TextString = REPLACE(TextString, '&quest;','?') ;
END IF ;
